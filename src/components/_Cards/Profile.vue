<template>
  <div id="profile"
    ...
    <div class="absolute bottom-0 left-0 right-0 flex w-full items-center justify-center space-x-6 p-4">
      <a :href="item.url" v-for="(item, index) in profile.contact" :key="item" :index="index"><i :class="item.icon"
          class="bx rounded-md bg-neutral-100 p-2"></i>
      </a>
    </div>
  </div>
</template>
